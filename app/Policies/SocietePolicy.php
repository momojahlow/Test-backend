...
class SocietePolicy
{
    ...
    {
        $hasEmploye=Societe::findOrFail($societe->id)->employes()->count() > 0;
        $hasInvit=Societe::findOrFail($societe->id)->invits()->count() > 0;
        return $user->type == 'admin' && !$hasEmploye  && !$hasInvit;
    }
